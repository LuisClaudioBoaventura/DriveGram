...
import { Headphones, X, Upload, Link as LinkIcon, Image as ImageIcon, Save, Check, Disc, Mic, Music2 } from 'lucide-react';
import { AudioShow, DriveItem } from '../types/index.js';

interface EditAudioModalProps {
  isOpen: boolean;
  onClose: () => void;
  audioShow: AudioShow | null;
  categories: string[];
  allFiles?: DriveItem[];
  onSave: (updated: AudioShow) => Promise<void>;
...
export const EditAudioModal: React.FC<EditAudioModalProps> = ({
  ...
  if (!isOpen || !audioShow) return null;

  const [title, setTitle] = useState(audioShow.title);
  const [showType, setShowType] = useState<'music_album' | 'podcast' | 'playlist'>(audioShow.showType || 'music_album');
  const [artist, setArtist] = useState(audioShow.artist || audioShow.host || '');
  const [category, setCategory] = useState(audioShow.category || 'Álbuns de Música');
  const [newCategoryName, setNewCategoryName] = useState('');
  const [isAddingNewCat, setIsAddingNewCat] = useState(false);
  const [genre, setGenre] = useState(audioShow.genre || '');
  const [description, setDescription] = useState(audioShow.description || '');
  const [coverImage, setCoverImage] = useState(audioShow.coverImage || PRESET_COVERS[0]);
      ...
      <div className="relative w-full max-w-2xl bg-white dark:bg-drive-darkSurface border border-gray-200 dark:border-drive-darkBorder rounded-3xl shadow-2xl overflow-hidden my-8">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-gray-800 bg-gradient-to-r from-emerald-600/10 via-teal-600/5 to-transparent">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-base font-bold text-gray-900 dark:text-gray-100">Editar Álbum / Podcast</h3>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5 max-h-[78vh] overflow-y-auto">
          ...
          <div className="grid grid-cols-3 gap-2">
            <button
              type="button"
              onClick={() => setShowType('music_album')}
              className={`p-2 rounded-xl border text-xs font-bold flex items-center justify-center gap-1.5 transition-all ${
                showType === 'music_album' ? 'bg-emerald-600/10 border-emerald-500 text-emerald-500 shadow-sm' : 'border-gray-200 dark:border-gray-700 text-gray-500'
              }`}
            >
            ...
            <button
              type="button"
              onClick={() => setShowType('podcast')}
              className={`p-2 rounded-xl border text-xs font-bold flex items-center justify-center gap-1.5 transition-all ${
                showType === 'podcast' ? 'bg-emerald-600/10 border-emerald-500 text-emerald-500 shadow-sm' : 'border-gray-200 dark:border-gray-700 text-gray-500'
              }`}
            >
            ...
            <button
              type="button"
              onClick={() => setShowType('playlist')}
              className={`p-2 rounded-xl border text-xs font-bold flex items-center justify-center gap-1.5 transition-all ${
                showType === 'playlist' ? 'bg-emerald-600/10 border-emerald-500 text-emerald-500 shadow-sm' : 'border-gray-200 dark:border-gray-700 text-gray-500'
              }`}
            >
          ...
          <div className="flex flex-col sm:flex-row gap-6 items-center sm:items-start">
            {/* Cover Preview */}
            <div className="relative w-36 aspect-square rounded-2xl overflow-hidden shadow-xl border-2 border-emerald-500/40 shrink-0 bg-black/40 group">
            ...
            <div className="flex-1 space-y-3 w-full">
              ...
              <div>
                <label className="block text-xs font-bold text-gray-700 dark:text-gray-300 mb-1">
                  {showType === 'podcast' ? 'Host / Apresentador' : 'Artista / Banda'}
                </label>
                <input
                  type="text"
                  value={artist}
                  onChange={(e) => setArtist(e.target.value)}
                  placeholder="Nome do artista ou podcaster"
                  className="w-full px-3.5 py-2 rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-drive-darkBg text-gray-900 dark:text-gray-100 text-xs font-medium focus:ring-2 focus:ring-emerald-500 focus:outline-none"
                />
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  {!isAddingNewCat ? (
                    <div className="flex gap-1">
                      <select
                        ...
                        className="flex-1 px-3 py-2 rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-drive-darkBg text-gray-900 dark:text-gray-100 text-xs font-medium focus:ring-2 focus:ring-emerald-500 focus:outline-none"
                      >
                        {categories.map(c => (
                    ...
                    <div className="flex gap-1">
                      <input
                        ...
                        placeholder="Nome"
                        className="flex-1 px-2.5 py-1.5 rounded-xl border border-emerald-500 bg-gray-50 dark:bg-drive-darkBg text-xs"
                      />
                      <button
                        type="button"
                        onClick={handleCreateCategory}
                        className="px-2.5 py-1 rounded-xl bg-emerald-600 text-white text-xs font-bold"
                      >
                        OK
                      </button>
                      <button
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="Ex: Rock, Tecnologia"
                    className="w-full px-3.5 py-2 rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-drive-darkBg text-gray-900 dark:text-gray-100 text-xs font-medium focus:ring-2 focus:ring-emerald-500 focus:outline-none"
                  />
                </div>
              </div>
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Descreva o álbum ou show..."
          ...
          <div className="space-y-3 pt-2 border-t border-gray-100 dark:border-gray-800">
            ...
            <div className="flex items-center gap-1 bg-gray-100 dark:bg-gray-800/60 p-1 rounded-xl border border-gray-200 dark:border-gray-700/60">
              <button
                ...
                className={`flex-1 py-1.5 text-xs font-bold rounded-lg transition-all flex items-center justify-center gap-1.5 ${
                  coverTab === 'gallery' ? 'bg-white dark:bg-drive-darkSurface text-emerald-500 shadow-sm' : 'text-gray-500 hover:text-gray-900 dark:hover:text-white'
                }`}
              >
              ...
              <button
                ...
                className={`flex-1 py-1.5 text-xs font-bold rounded-lg transition-all flex items-center justify-center gap-1.5 ${
                  coverTab === 'upload' ? 'bg-white dark:bg-drive-darkSurface text-emerald-500 shadow-sm' : 'text-gray-500 hover:text-gray-900 dark:hover:text-white'
                }`}
              >
              ...
              <button
                ...
                className={`flex-1 py-1.5 text-xs font-bold rounded-lg transition-all flex items-center justify-center gap-1.5 ${
                  coverTab === 'url' ? 'bg-white dark:bg-drive-darkSurface text-emerald-500 shadow-sm' : 'text-gray-500 hover:text-gray-900 dark:hover:text-white'
                }`}
              >
              ...
              {folderImageFiles.length > 0 && (
                <button
                  ...
                  className={`flex-1 py-1.5 text-xs font-bold rounded-lg transition-all flex items-center justify-center gap-1.5 ${
                    coverTab === 'folder' ? 'bg-white dark:bg-drive-darkSurface text-emerald-500 shadow-sm' : 'text-gray-500 hover:text-gray-900 dark:hover:text-white'
                  }`}
                >
                  <Disc className="w-3.5 h-3.5" />
            ...
            {coverTab === 'upload' && (
              <div
                ...
                className="border-2 border-dashed border-gray-300 dark:border-gray-700 hover:border-emerald-500 dark:hover:border-emerald-500 rounded-2xl p-6 flex flex-col items-center justify-center cursor-pointer transition-colors text-center bg-gray-50/50 dark:bg-drive-darkBg/50"
              >
                <input
                  ref={fileInputRef}
                  type="file"
                  ...
                  return (
                    <button
                      ...
                      className={`relative rounded-xl overflow-hidden aspect-square border-2 transition-all ${
                        coverImage === url ? 'border-emerald-500 ring-2 ring-emerald-500/30 scale-105' : 'border-transparent opacity-70 hover:opacity-100'
                      }`}
                    >
